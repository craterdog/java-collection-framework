...
public class MapTest {
    ...
    public void testMapOperations() {
        ...
        Map<Integer, Integer> map = Map.concatenation(emptyMap, firstMap);
        assertEquals("  The resulting map from the concatenation is incorrect.", firstMap, map);
        map = Map.concatenation(firstMap, emptyMap);
        assertEquals("  The resulting map from the concatenation is incorrect.", firstMap, map);
        map = Map.concatenation(firstMap, secondMap);
        ...
        map = Map.reduction(emptyMap, keys);
        assertEquals("  The resulting map from the reduction is incorrect.", emptyMap, map);
        keys = new Set<>(second);
        map = Map.reduction(emptyMap, keys);
        assertEquals("  The resulting map from the reduction is incorrect.", emptyMap, map);
        keys = new Set<>(empty);
        map = Map.reduction(firstMap, keys);
        assertEquals("  The resulting map from the reduction is incorrect.", emptyMap, map);
        keys = new Set<>(second);
        map = Map.reduction(thirdMap, keys);
